...
def main() -> int:
    ...
    plot_time(base, iq, fs)
    plot_psd(base, iq, fs, fc)
    plot_spectrogram(base, spec_db, time_s, freq_hz, fc, signal_boxes, energy_boxes, overlay=False)
    plot_occupancy(base, time_s, freq_hz, fc, signal_boxes, energy_boxes, overlay=False)
    plot_spectrogram(base, spec_db, time_s, freq_hz, fc, signal_boxes, energy_boxes, overlay=True)
    plot_occupancy(base, time_s, freq_hz, fc, signal_boxes, energy_boxes, overlay=True)
    zoom_time_ms, zoom_freq_mhz = compute_zoom_limits(signal_boxes, fc, time_s, freq_hz)
    plot_spectrogram(
        base,
        spec_db,
        time_s,
        ...
        overlay=True,
        suffix="zoom_overlay",
        xlim_ms=zoom_time_ms,
        ylim_mhz=zoom_freq_mhz,
    )
    plot_occupancy(
        base,
        time_s,
...
def plot_time(base: Path, iq: np.ndarray, fs: float) -> None:
    ...
    fig.tight_layout()
    fig.savefig(f"{base}_time.png", dpi=160)
    plt.close(fig)


def plot_psd(base: Path, iq: np.ndarray, fs: float, fc: float, fft_len: int = 131072) -> None:
    ...
    ax.grid(True, alpha=0.2)
    fig.tight_layout()
    fig.savefig(f"{base}_psd.png", dpi=160)
    plt.close(fig)
...
def plot_spectrogram(
    ...
    ylim_mhz: tuple[float, float] | None = None,
) -> None:
    ...
    fig.tight_layout()
    fig.savefig(f"{base}_{suffix}.png", dpi=180)
    plt.close(fig)
...
def plot_occupancy(
    ...
    fig.tight_layout()
    if suffix is None:
        suffix = "fullband_occupancy" if overlay else "occupancy"
    fig.savefig(f"{base}_{suffix}.png", dpi=180)
    plt.close(fig)
...
def add_family_legend(ax, color_map: dict[str, str]) -> None:
    handles = [
        Line2D([0], [0], color=color, lw=2, label=family_label(family))
...
def metadata_family(md: dict) -> str:
    for key in ("modulation", "protocol"):
        ...
        if value and value not in {"unknown", "no_answer"}:
            return value
    instance_name = normalize_family(str(md.get("instance_name", "")))
    return instance_name or "unknown"
...
def verify_alignment(iq: np.ndarray, metadata: dict, spec_db: np.ndarray, time_s: np.ndarray, freq_hz: np.ndarray, signal_boxes: list[Box], energy_boxes: list[Box], duration_s: float) -> dict:
    ...
    signal_empty = [
        ...
        if box_peak(spec_db, time_s, freq_hz, box) < global_floor + 6.0
    ]
    energy_empty = [
        box.label
    ...
    for p in percentiles:
        threshold = np.percentile(spec_db, p)
        ...
        percentile_stats[f"top_{100.0 - p:.1f}_percent"] = {
    ...
    per_energy_inband_ratios = energy_inband_ratios(iq, metadata, energy_boxes)
    finite_energy_ratios = [x for x in per_energy_inband_ratios if np.isfinite(x)]
    min_energy_ratio = float(min(finite_energy_ratios)) if finite_energy_ratios else math.nan

    if full_span_signal:
        verdict = "Visual pass" if not signal_empty else "Inconclusive"
    elif signal_empty or energy_empty:
        verdict = "Visual fail"
    elif finite_energy_ratios and min_energy_ratio >= 0.9:
        verdict = "Visual pass"
    ...
    return {
        ...
        "per_energy_inband_ratios": per_energy_inband_ratios,
        "min_energy_inband_ratio": min_energy_ratio,
...
def is_nearly_full_span(box: Box, duration_s: float, freq_hz: np.ndarray) -> bool:
    ...
    return (
        (box.time_stop - box.time_start) >= 0.95 * duration_s
        and (box.freq_hi - box.freq_lo) >= 0.95 * total_bw
    )
...
def energy_inband_ratios(iq: np.ndarray, metadata: dict, energy_boxes: list[Box]) -> list[float]:
    ...
    for box in energy_boxes:
        ...
        if stop <= start:
            ...
            continue
        chunk = iq[start:stop]
        total_pow = float(np.vdot(chunk, chunk).real)
        if total_pow <= 0:
            ratios.append(math.nan)
            continue
